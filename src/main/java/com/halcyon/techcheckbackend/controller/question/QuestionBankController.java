...
import com.halcyon.techcheckbackend.model.dto.questionBank.QuestionBankAddRequest;
import com.halcyon.techcheckbackend.model.dto.questionBank.QuestionBankEditRequest;
import com.halcyon.techcheckbackend.model.dto.questionBank.QuestionBankQueryRequest;
import com.halcyon.techcheckbackend.model.dto.questionBank.QuestionBankUpdateRequest;
...
import com.halcyon.techcheckbackend.service.question.QuestionBankService;
...
public class QuestionBankController {
    ...
    public BaseResponse<Long> addQuestionBank(@RequestBody QuestionBankAddRequest questionBankAddRequest, HttpServletRequest request) {
        ThrowUtils.throwIf(questionBankAddRequest == null, ErrorCode.PARAMS_ERROR, "参数不能为空");
        ...
        ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR, "创建题库失败");
        // 返回新写入的数据 id
    ...
    public BaseResponse<Boolean> deleteQuestionBank(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        // 仅本人或管理员可删除
        boolean notAuthorized = !oldQuestionBank.getUserId().equals(user.getId()) && !userService.isAdmin(request);
    ...
    public BaseResponse<Boolean> updateQuestionBank(@RequestBody QuestionBankUpdateRequest questionBankUpdateRequest) {
        ThrowUtils.throwIf(questionBankUpdateRequest == null || questionBankUpdateRequest.getId() <= 0, ErrorCode.PARAMS_ERROR, "参数错误");
        ...
        ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR, "更新题库失败");
        return ResultUtils.success(true);
    }
    ...
    public BaseResponse<Page<QuestionBankVO>> listMyQuestionBankVOByPage(@RequestBody QuestionBankQueryRequest questionBankQueryRequest,
                                                                         HttpServletRequest request) {
        ThrowUtils.throwIf(questionBankQueryRequest == null, ErrorCode.PARAMS_ERROR, "参数错误");
        // 补充查询条件，只查询当前登录用户的数据
        User loginUser = userService.getLoginUser(request);
    ...
    public BaseResponse<Boolean> editQuestionBank(@RequestBody QuestionBankEditRequest questionBankEditRequest, HttpServletRequest request) {
        ThrowUtils.throwIf(questionBankEditRequest == null || questionBankEditRequest.getId() <= 0, ErrorCode.PARAMS_ERROR, "参数错误");
        //在此处将实体类和 DTO 进行转换
        QuestionBank questionBank = new QuestionBank();
        ...
        QuestionBank oldQuestionBank = questionBankService.getById(id);
        ThrowUtils.throwIf(oldQuestionBank == null, ErrorCode.NOT_FOUND_ERROR, "题库不存在");
        boolean notAuthorized = !oldQuestionBank.getUserId().equals(loginUser.getId()) && !userService.isAdmin(request);
        ThrowUtils.throwIf(notAuthorized, ErrorCode.NO_AUTH_ERROR, "无权限");
        // 操作数据库
        boolean result = questionBankService.updateById(questionBank);
